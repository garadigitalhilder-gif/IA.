...
module = st.sidebar.radio(
    ...
    [
        ...
        "✍️ Módulo 3: Documentación y Uso de IA"
    ]
)
...
if module == "🎮 Módulo 1: Simulador 2D (Regresión Lineal)":
    ...
    with col_side:
        st.markdown("### 🛠️ Control de Datos y Modelo")
        ...
        st.markdown("**✏️ Editar Puntos Directamente (X e Y entre 0 y 1, Clase 0 o 1):**")
        ...
        # Opciones visuales
        st.markdown("### 👁️ Opciones del Gráfico")
    ...
    with col_plot:
        ...
        st.warning("⚠️ **Impacto de Outliers:** Carga la plantilla **'Con Outliers'** y entrena el modelo. Observa cómo los dos puntos lejanos a la derecha fuerzan a la línea a subir, moviendo la frontera de decisión hacia la izquierda, lo que causa clasificaciones incorrectas en la zona central. Esto demuestra que minimizar errores cuadráticos continuos no es el enfoque ideal para clasificación binaria.")
        ...
        if st.session_state.m2_raw_data is not None:
            ...
            if len(num_cols) < 2:
                ...
                with col_sel1:
                    ...
                    positive_class = None
                    if is_multiclass:
                        st.info(f"ℹ️ El target '{target}' contiene {len(unique_vals)} clases ({unique_vals}). Para aplicar Regresión Logística Binaria, selecciona cuál de estas clases consideras como Clase Positiva (1). Las demás se agruparán automáticamente como Clase Negativa (0).")
                        ...
                        if len(unique_vals) != 2 or unique_vals != [0, 1]:
                            st.warning(f"⚠️ ¡Cuidado! Regresión Logística requiere una variable binaria (0 y 1). Los valores únicos detectados en '{target}' son: {unique_vals}. Los mapearemos automáticamente a [0, 1].")
...
# ==============================================================================
elif module == "✍️ Módulo 3: Documentación y Uso de IA":
    st.markdown("## ✍️ Módulo 3: Documentación y Uso de IA Generativa")
    ...
    tab_doc1, tab_doc2, tab_doc3, tab_doc4 = st.tabs([
        "1. Qué fue generado por IA", 
        "2. Qué fue corregido por el estudiante", 
        "3. Qué fue comprendido",
        "4. Guía de Despliegue en la Nube (Link Único)"
    ])

    with tab_doc1:
        st.markdown("#### ⚙️ Código y Elementos Generados por la IA")
        st.markdown("""
    ...
    with tab_doc2:
        st.markdown("#### 🛠️ Ajustes y Correcciones Realizados por el Estudiante")
        st.markdown("""
    ...
    with tab_doc3:
        st.markdown("#### 🧠 Conceptos y Fundamentos Comprendidos")
        st.markdown("""
    ...
    with tab_doc4:
        st.markdown("#### ☁️ Cómo subir tu app a internet gratis (Link Único)")
        st.markdown("""
